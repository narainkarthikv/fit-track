...
const NotificationDropdown = () => {
  ...
  return (
    <Dropdown as={ButtonGroup} align='start'>
      ...
      <Button variant='primary' onClick={markAllAsRead}>Read All</Button>
      ...
      <Dropdown.Menu align='end' className='p-2'>
        ...
        {notifications.map(notification => (
          ...
          >
            <span className='flex-wrap' onClick={() => toggleNotificationReadStatus(notification.id)}>
              {notification.message}
            </span>
            <div>
              {notification.read ? <FaEnvelopeOpen className='me-2' /> : <FaEnvelope className='me-2' />}
              <FaTrashAlt onClick={() => deleteNotification(notification.id)} />
            </div>
          </Dropdown.Item>
        ))}
        {notifications.length === 0 && <Dropdown.Item className='text-center'>No notifications</Dropdown.Item>}
      </Dropdown.Menu>
    </Dropdown>
  );
};
